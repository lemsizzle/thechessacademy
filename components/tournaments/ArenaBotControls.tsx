...
export function ArenaBotControls({ arena, adminActionToken, onChange }: { arena: InternalArena; adminActionToken: string; onChange: () => Promise<void> }) {
  ...
  return <section className="mt-4 min-w-0 rounded-lg border border-violet-300/25 bg-violet-300/10 p-4" aria-label="Arena bots">
    ...
    <p className="mt-2 text-xs text-slate-400">Skill ratings are estimates. You can add multiple bots at different levels. Bots only play students.</p>
    ...
    <div className="mt-4 space-y-3">{bots.map((entry) => {
      ...
      return <div key={bot.id} className="min-w-0 rounded-lg border border-white/10 bg-slate-950/60 p-3">
        <div className="flex min-w-0 items-center gap-2"><BotPortrait src={preset?.portrait ?? "/bots/zippy-knight.png"} /><div className="min-w-0"><p className="truncate text-sm font-black text-white">{bot.name} <span className="text-xs text-violet-200">BOT</span></p><p className="text-xs text-slate-400">{entry.status === "playing" ? "Playing · changes apply next game" : "Ready for matchmaking"}</p></div></div>
        <label className="mt-3 grid min-w-0 gap-1 text-xs text-slate-300">Skill for {bot.name}<select className={field} value={selected} disabled={Boolean(pending)} onChange={(event) => setEdits((value) => ({ ...value, [bot.id]: event.target.value }))}>{BOT_DIFFICULTIES.map((level) => <option key={level.id} value={level.id}>{level.name} · ~{level.estimatedRating}</option>)}</select></label>
        <div className="mt-2 flex flex-wrap gap-2"><Button type="button" variant="secondary" disabled={Boolean(pending) || selected === bot.difficultyId} onClick={() => void change("PATCH", { botId: bot.id, name: bot.name, difficultyId: selected }, bot.id)}>Save Skill</Button><Button type="button" variant="ghost" disabled={Boolean(pending) || entry.status === "playing"} onClick={() => setRemovingId(bot.id)}>Remove</Button></div>
        {removingId === bot.id ? <div className="mt-3 rounded-md border border-rose-300/30 bg-rose-300/10 p-3"><p className="text-sm text-rose-100">Remove {bot.name} from this Arena?</p><div className="mt-2 flex flex-wrap gap-2"><Button type="button" variant="secondary" disabled={Boolean(pending) || entry.status === "playing"} onClick={() => void change("DELETE", { botId: bot.id }, bot.id)}>Confirm Remove</Button><Button type="button" variant="ghost" disabled={Boolean(pending)} onClick={() => setRemovingId(null)}>Keep Bot</Button></div></div> : null}
      </div>;
    })}</div>
  </section>;
}
