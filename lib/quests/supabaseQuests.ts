...
const botIds = new Set(BOT_DIFFICULTIES.map((bot) => bot.id));
...
function questPayload(quest: Quest) {
  return {
    ...
    required_opponent_id: supportsComputerOpponentFilter(quest.conditionType) ? quest.requiredOpponentId?.trim() || null : null,
