...
const localPath = "work/avatar-assets/glasses/gojo-blindfold.png";
const bucket = "avatar-assets";
const objectPath = `glasses/gojo-blindfold-${version}.png`;
const item = {
  name: "Gojo's Blindfold",
  slug: "gojos-blindfold",
  description: "A sleek black sorcerer's blindfold with layered folds and a secure temple-to-temple wrap.",
  category: "glasses",
  rarity: "Legendary",
  price: 200,
  layer_order: 55,
...
async function main() {
  ...
  if (localMetadata.width !== 1024 || localMetadata.height !== 1024 || !localMetadata.hasAlpha) {
    throw new Error("Gojo's Blindfold must be a transparent 1024x1024 PNG.");
  }
  ...
  if (!response.ok) throw new Error(`Published blindfold returned HTTP ${response.status}.`);
  ...
  if (remoteMetadata.width !== 1024 || remoteMetadata.height !== 1024 || !remoteMetadata.hasAlpha) {
    throw new Error("The published blindfold failed the remote PNG metadata check.");
  }
