...
const source = path.join(__dirname, "gojo-blindfold-source.png");
const output = path.join(__dirname, "gojo-blindfold.png");
const preview = path.join(__dirname, "gojo-blindfold-preview.png");
const placement = { left: 232, top: 380, width: 560, height: 160 };
...
async function main() {
  const blindfold = await sharp(source)
  ...
  const canvas = await sharp({
    ...
    .composite([{ input: blindfold, left: placement.left, top: placement.top }])
