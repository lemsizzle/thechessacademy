...
const localPath = "work/avatar-assets/backgrounds/chess-domain.png";
const bucket = "avatar-assets";
const objectPath = `backgrounds/chess-domain-${version}.png`;
const item = {
  name: "Chess Domain",
  slug: "chess-domain",
  description: "An infinite supernatural chess realm guarded by towering spectral pieces and fractured board energy.",
  category: "background",
  rarity: "Legendary",
  price: 400,
  layer_order: 0,
...
async function main() {
  ...
  if (remoteMetadata.width !== 1024 || remoteMetadata.height !== 1024 || remoteMetadata.format !== "png") {
    throw new Error("The published Chess Domain failed the remote PNG metadata check.");
  }
