...
const reuseExistingAsset = process.argv.includes("--reuse-existing");
if (!/^v\d+$/.test(version ?? "")) {
  throw new Error("Pass a version such as v1 as the first argument.");
}

const localPath = "work/avatar-assets/clothing/akatsuki-robe.png";
const bucket = "avatar-assets";
const objectPath = `clothing/akatsuki-robe-${version}.png`;
const item = {
  name: "Akatsuki Robe",
  slug: "akatsuki-robe",
  description: "A black high-collar robe marked with the Akatsuki's red cloud pattern.",
  category: "clothing",
  rarity: "Legendary",
  price: 300,
  layer_order: 40,
...
async function main() {
  ...
  if (!response.ok) throw new Error(`Published robe returned HTTP ${response.status}.`);
  ...
  if (remoteMetadata.width !== 1024 || remoteMetadata.height !== 1024 || !remoteMetadata.hasAlpha) {
    throw new Error("The published robe failed the remote PNG metadata check.");
  }
