...
export async function POST(request: Request) {
  const body = await request.json() as { award?: PendingTournamentAward; xpAmount?: number; teacherNote?: string };
  if (!body.award || body.award.status !== "pending") {
    return NextResponse.json({ error: "A pending tournament award is required." }, { status: 400 });
  }
  return NextResponse.json({ award: approveTournamentAward(body.award, body.xpAmount, body.teacherNote) });
}
