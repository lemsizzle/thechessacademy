...
import { findStarWarsSolution, initialStarWarsState, starWarsPuzzleForScore } from "@/lib/puzzle-training/starWars";
...
describe("Star Wars server verification", () => {
  ...
  it("rejects incomplete, invalid, and oversized route submissions", () => {
    ...
    expect(() => parseStarWarsRoutes([Array.from({ length: 8 }, () => ({ from: "a1", to: "a2" }))])).toThrow(/invalid length/i);
  });
