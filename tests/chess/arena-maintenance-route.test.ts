...
beforeEach(() => { vi.stubEnv("ARENA_MAINTENANCE_SECRET", ""); });
...
it("uses a dedicated Arena secret without changing other cron jobs", async () => {
  vi.stubEnv("CRON_SECRET", "existing-cron-secret");
  vi.stubEnv("ARENA_MAINTENANCE_SECRET", "arena-only-secret");
  run.mockResolvedValue({ checked: 0 });
  expect((await POST(new Request("http://localhost", { headers: { authorization: "Bearer existing-cron-secret" } }))).status).toBe(401);
  expect((await POST(new Request("http://localhost", { headers: { authorization: "Bearer arena-only-secret" } }))).status).toBe(200);
});
