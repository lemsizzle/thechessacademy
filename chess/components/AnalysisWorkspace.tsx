...
export function AnalysisWorkspace({ initialTree, title, subtitle, editable = true, saveStatus = "idle", saveMessage = "", onTreeChange, actions, gameMode = false, canManageReferenceEvaluations = false, canManageGuidedExercises = false, guidedStudentMode = false, guidedExerciseContext, reviewColor, reviewGameId }: Props) {
  ...
  const mistakeReview = useMistakeReview(originalTreeRef.current, reviewColor, reviewGameId);
      ...
      <div className="grid min-w-0 items-start gap-4 xl:grid-cols-[minmax(0,720px)_minmax(360px,520px)] xl:justify-center">
        ...
        <aside className="min-w-0 space-y-4 xl:sticky xl:top-4">
          ...
          <Card className="p-4">
            ...
            <button type="button" ref={activeId === tree.rootId ? activeButtonRef : undefined} aria-current={activeId === tree.rootId ? "step" : undefined} onClick={() => selectPosition(tree.rootId)} className={`mt-3 w-full rounded-md px-3 py-2 text-left text-sm font-bold ${activeId === tree.rootId ? "bg-cyan-300 text-slate-950 ring-2 ring-cyan-100 shadow-lg shadow-cyan-500/30" : "bg-white/5 text-slate-300"}`}>Starting position {tree.nodes[tree.rootId].guidedExercise && <span title="Guided exercise" className="text-violet-300">◆</span>}</button>
            <div ref={moveTreeRef} className="scrollbar-soft mt-2 max-h-72 space-y-1 overflow-y-auto rounded-md border border-white/10 bg-slate-950/70 p-2" role="tree" aria-keyshortcuts="ArrowLeft ArrowRight Home End">
              {moveRows.map((row) => <div key={row.moveNumber} role="group" aria-label={`Move ${row.moveNumber}`} className="grid grid-cols-2 gap-1">
                {[row.whiteNodeId, row.blackNodeId].map((id, column) => {
                  ...
                  return <button key={id} type="button" ref={activeId === id ? activeButtonRef : undefined} role="treeitem" aria-selected={activeId === id} aria-current={activeId === id ? "step" : undefined} onClick={() => selectPosition(id)} className={`min-w-0 rounded px-2 py-1.5 text-left text-sm font-bold ${activeId === id ? "bg-cyan-300 text-slate-950 ring-2 ring-inset ring-cyan-100 shadow-md shadow-cyan-500/30" : "text-slate-200 hover:bg-white/5"}`}>
              ...
              {variationRows.length ? <div className="border-t border-white/10 pt-2">
                ...
                return <div key={id} className="group flex items-center gap-1" style={{ paddingLeft: `${Math.min(depth, 7) * 14}px` }}>
                  <button type="button" ref={activeId === id ? activeButtonRef : undefined} role="treeitem" aria-selected={activeId === id} aria-current={activeId === id ? "step" : undefined} onClick={() => selectPosition(id)} className={`min-w-0 flex-1 rounded px-2 py-1.5 text-left text-sm ${activeId === id ? "bg-cyan-300 font-black text-slate-950 ring-2 ring-inset ring-cyan-100 shadow-md shadow-cyan-500/30" : "text-amber-100 hover:bg-white/5"}`}>
