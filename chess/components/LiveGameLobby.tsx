    ...
    <div className="space-y-5">
      ...
      <Card className="p-5 sm:p-6">
        <div className="grid gap-5 lg:grid-cols-[1fr_240px_auto] lg:items-end">
          <div><p className="text-xs font-black uppercase tracking-wider text-emerald-200">Quick matchmaking</p><h2 className="mt-1 text-2xl font-black text-white">Find an Academy opponent</h2><p className="mt-2 text-sm text-slate-400">Choose a clock and we’ll find an available student with a suitable match.</p></div>
          <div><label htmlFor="match-clock" className="text-sm font-bold text-slate-200">Clock</label><select id="match-clock" value={matchTimeControlId} disabled={matchmaking?.status === "waiting"} onChange={(event) => setMatchTimeControlId(event.target.value)} className="mt-2 w-full rounded-md border border-white/10 bg-slate-900 px-3 py-2 text-white">{TIME_CONTROLS.filter((control) => control.initialMs !== null).map((control) => <option key={control.id} value={control.id}>{control.name}</option>)}</select></div>
        ...
        {matchmaking?.status === "waiting" ? <p className="mt-4 rounded-md border border-emerald-300/25 bg-emerald-300/10 p-3 text-sm font-bold text-emerald-100" aria-live="polite">Searching for a {TIME_CONTROLS.find((item) => item.id === matchmaking.timeControlId)?.name ?? ""} opponent…</p> : null}
      </Card>
