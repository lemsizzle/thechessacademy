...
function parseArguments() {
  ...
  const unknownOption = args.find((arg) => arg.startsWith("--") && !["--fast", "--sample", "--dry-run", "--new-only"].includes(arg));
  ...
  if (!source) {
    throw new Error("Usage: npm run import:lichess-puzzles -- <path-to-lichess_db_puzzle.csv.zst> [--fast|--sample] [--dry-run] [--new-only]");
  }
  return {
...
async function upsertRows(supabase: SupabaseClient, rows: ImportRow[], batchSize: number, newOnly: boolean) {
  let inserted = 0;
  let updated = 0;
  for (let index = 0; index < rows.length; index += batchSize) {
    const batch = rows.slice(index, index + batchSize);
    if (newOnly) {
      // Ignore conflicts even if another import inserted the same ID after our read.
      inserted += await insertNewPuzzleBatch(supabase, batch);
      console.log(`Processed ${Math.min(index + batch.length, rows.length).toLocaleString()} / ${rows.length.toLocaleString()} new candidates.`);
      continue;
    }
...
async function main() {
  const { filePath, mode, dryRun, newOnly } = parseArguments();
  ...
  if (ratingMin > ratingMax) throw new Error("Minimum rating must not exceed maximum rating.");
  const supabase = !dryRun || newOnly ? createClient(
    requireEnvironment("NEXT_PUBLIC_SUPABASE_URL"),
    requireEnvironment("SUPABASE_SERVICE_ROLE_KEY"),
    { auth: { persistSession: false, autoRefreshToken: false } }
  ) : null;
  const excludedIds = newOnly ? await loadExistingPuzzleIds(supabase!) : new Set<string>();
  if (newOnly) console.log(`Keeping ${excludedIds.size.toLocaleString()} existing puzzle IDs unchanged.`);
  ...
  for await (const raw of createCsvStream(filePath, () => { csvRowsSkipped += 1; })) {
    ...
    const matchingThemes = targetThemesFor({ themes });
    if (!matchingThemes.length) continue;
    if (excludedIds.has(puzzleId)) { existingSkipped += 1; continue; }
    if (mode === "fast" && matchingThemes.every(theme => selectedCounts[theme] >= perTheme)) continue;
  ...
  const manifestPath = process.env.PUZZLE_IMPORT_MANIFEST_PATH?.trim();
  if (manifestPath) {
    const destination = resolve(process.cwd(), manifestPath);
    mkdirSync(dirname(destination), { recursive: true });
    writeFileSync(destination, JSON.stringify({ source: filePath, ratingMin, ratingMax, rows }));
    console.log(`Saved validated selection to ${destination}`);
  }
  let inserted = 0;
  let updated = 0;
  if (!dryRun) {
    ({ inserted, updated } = await upsertRows(supabase!, rows, batchSize, newOnly));
  }
  ...
  console.log(`Existing puzzle IDs skipped: ${existingSkipped.toLocaleString()}`);
  ...
  for (const [name, min, max] of [["Beginner", 600, 999], ["Improver", 1000, 1399], ["Intermediate", 1400, 1799], ["Advanced+", 1800, Infinity]] as const) {
    console.log(`${name} selected: ${rows.filter(row => row.rating >= min && row.rating <= max).length.toLocaleString()}`);
  }
