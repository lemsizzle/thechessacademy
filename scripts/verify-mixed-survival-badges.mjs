...
await db.exec("alter table coin_transactions drop constraint force_failure; alter table student_badges add column survival_session_id uuid");
await db.exec(await readFile(new URL("../supabase/migrations/20260909022308_mixed_survival_chaos_mastery.sql", import.meta.url), "utf8"));
await db.exec(await readFile(new URL("../supabase/migrations/20260909022308_mixed_survival_chaos_mastery.sql", import.meta.url), "utf8"));
assert.equal((await db.query("select count(*)::int n from survival_tactical_badge_rules where selected_theme='mixed'")).rows[0].n,4);
assert.deepEqual(await award(student,sessions.mixed),[], "Old mixed rounds remain ineligible");
const mixedStudent=(await db.query("insert into students default values returning id")).rows[0].id;
const mixedSession="20000000-0000-4000-8000-000000000001";
for(let i=0;i<40;i++){
 await attempt(pinPuzzles[i],{studentId:mixedStudent,session:mixedSession,theme:"mixed"});
 const awards=await award(mixedStudent,mixedSession);
 if(expected.has(i+1)){
  assert.equal(awards.length,1);
  assert.equal(awards[0].tier,expected.get(i+1)[0]);
  assert.equal(awards[0].coins,expected.get(i+1)[1]);
  assert.match(awards[0].name,/Chaos Mastery/);
 } else assert.deepEqual(awards,[]);
 assert.deepEqual(await award(mixedStudent,mixedSession),[]);
}
assert.equal((await db.query("select academy_coins from student_wallets where student_id=$1",[mixedStudent])).rows[0].academy_coins,360);
assert.equal((await db.query("select count(*)::int n from student_badges where student_id=$1 and survival_session_id=$2",[mixedStudent,mixedSession])).rows[0].n,4);
assert.equal((await db.query("select count(*)::int n from student_badges sb join survival_tactical_badge_rules r on r.badge_id=sb.badge_id where sb.student_id=$1 and r.selected_theme <> 'mixed'",[mixedStudent])).rows[0].n,0);
const boundaryStudent=(await db.query("insert into students default values returning id")).rows[0].id;
const boundarySession="20000000-0000-4000-8000-000000000002";
for(let i=0;i<9;i++)await attempt(pinPuzzles[i],{studentId:boundaryStudent,session:boundarySession,theme:"mixed"});
await attempt(pinPuzzles[0],{studentId:boundaryStudent,session:boundarySession,theme:"mixed"});
for(let i=9;i<20;i++)await attempt(pinPuzzles[i],{studentId:boundaryStudent,session:boundarySession,theme:"mixed",hints:1});
assert.deepEqual(await award(boundaryStudent,boundarySession),[]);
await attempt(pinPuzzles[20],{studentId:boundaryStudent,session:boundarySession,theme:"mixed"});
assert.equal((await award(boundaryStudent,boundarySession))[0].tier,"Bronze");
await db.query("update student_puzzle_attempts set attempted_at='2020-01-01' where student_id=$1 and puzzle_id=$2",[boundaryStudent,pinPuzzles[0]]);
for(let i=21;i<40;i++)await attempt(pinPuzzles[i],{studentId:boundaryStudent,session:boundarySession,theme:"mixed"});
assert.deepEqual(await award(boundaryStudent,boundarySession),[], "Resumed pre-launch rounds cannot earn higher tiers");
const splitStudent=(await db.query("insert into students default values returning id")).rows[0].id;
for(let i=0;i<9;i++){
 await attempt(pinPuzzles[i],{studentId:splitStudent,session:sessions.partialA,theme:"mixed"});
 await attempt(pinPuzzles[i+10],{studentId:splitStudent,session:sessions.partialB,theme:"mixed"});
}
assert.deepEqual(await award(splitStudent,sessions.partialA),[]);
assert.deepEqual(await award(splitStudent,sessions.partialB),[]);
for(const role of ["anon","authenticated"])
 assert.equal((await db.query("select has_function_privilege($1,'award_survival_tactical_badges(uuid,uuid)','execute') allowed",[role])).rows[0].allowed,false);
await db.close();
console.log("PASS: focused regression plus new-only Mixed Themes, all thresholds, exact coins, hints, distinct puzzles, no cumulative rounds, attribution, replay and permissions.");
